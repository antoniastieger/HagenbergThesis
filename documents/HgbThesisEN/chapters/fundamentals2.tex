...
% !!!! The configuration of your CLLocationManager object affects which radios the system uses and your app's power consumption.
% https://medium.com/@matheusqcardoso98/ios-retrieving-the-users-location-and-keeping-it-optimal-ee1d59c7fe19
% CoreLocation: iOS' location-related utilities can be accessed through the CLLocationManager class
...
% var accuracyAuthorization: CLAccuracyAuthorization A value that indicates the level of location accuracy the app has permission to use. or The accuracy of a geographical coordinate. typealias of desiredAccuracy
% class func isMonitoringAvailable(for: AnyClass) -> Bool Returns a Boolean value indicating whether the device supports region monitoring using the specified class.
% var distanceFilter: CLLocationDistance The minimum distance in meters the device must move horizontally before an update event is generated. Set the distanceFilter property to the largest possible value that gives you the information you need. Higher values let the system turn off radio hardware more frequently.
% let CLLocationDistanceMax: CLLocationDistance A constant indicating the maximum distance.
% let kCLDistanceFilterNone: CLLocationDistance A constant indicating that all movement should be reported.
% var desiredAccuracy: CLLocationAccuracy The accuracy of the location data that your app wants to receive. Set the desiredAccuracy property to the lowest possible value that gives you the information you need. Lower accuracy values let the system use more power-efficient hardware. Lower values also let the system turn off hardware sooner.
% var monitoredRegions: Set<CLRegion> The set of shared regions monitored by all location-manager objects.
